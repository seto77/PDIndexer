...
## P–V–T tables used for spline interpolation {#pvt-tables}

Among the sources listed in [Per-source formulas](#per-source), some have no closed-form equation and instead obtain pressure by **spline-interpolating a published P–V–T table**. These tables are not included in the external explanatory page ([yseto.net](https://yseto.net/misc/misc-4/misc-4-1)), so the raw data used by the implementation is reproduced verbatim below (source: `EOS.cs` / `FormEOS.cs`).
