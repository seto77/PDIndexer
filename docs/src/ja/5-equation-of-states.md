...
<!-- 260605Cl: スプライン補間EOS出典のP-V-Tテーブル(yseto.net未収録分)を追加 -->

## スプライン補間に用いる P–V–T テーブル {#pvt-tables}

[出典別の計算式](#per-source) のうち、解析的な数式を持たず、公表された **P–V–T 数表をスプライン補間** して圧力を求める出典を以下にまとめます。これらの数表は外部解説ページ（[yseto.net](https://yseto.net/misc/misc-4/misc-4-1)）には収録されていないため、実装が用いている元データをそのまま掲載します（出典: `EOS.cs` / `FormEOS.cs`）。
...
??? note "アルゴン Ar — Ross et al. (1986)（273 K 等温; モル体積→圧力）"
...
??? note "鉛 Pb — Strässle et al. (2014)（T依存パラメータ表）"
